...
def parser() -> argparse.ArgumentParser:
    ...
    for command in ("sample", "backtest", "fetch-fx", "paper-step"):
...
def execute(args) -> dict:
    ...
    if args.command == "backtest":
        return save_run(read_bars(args.data, cfg), cfg, args.output)
    with httpx.Client(follow_redirects=False) as client:
        ...
        return paper_step(frame, quote, cfg, args.database)
